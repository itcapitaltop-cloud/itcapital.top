<div x-data="{ isModalBuyPackageActive: false }" x-on:itc:packages-refresh.window="$wire.$refresh()" class="relative">
    ...
    <x-ui.card-tabs
        ...
        <x-slot name="packages">
            <div>
                <x-widget.modal condition-name="isModalBuyPackageActive" max-width="sm" class="p-4 md:min-w-[350px] min-w-[250px]">
                    <x-bg.section-slim class="!px-1 !py-2">
                        <div class="flex items-center justify-between">
                            <h3 class="text-white font-dela text-[20px] ">{{ __('livewire_itc_purchase_packages') }}</h3>
                    ...
                    <x-bg.section-slim class="!px-1 !py-2">
                        <form wire:submit="buyPackage" x-on:bought.window="isModalBuyPackageActive = false">
                            <x-ui.input name="amount" placeholder="{{ __('livewire_itc_minimum_itc_placeholder') }}" validate="number" input-class="py-[5px] px-[12px]">
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="font-dela text-[16px] leading-[40px]">
                            <th class="py-1.5 pr-4 font-thin">{{ __('livewire_dashboard_index_old_transactions_date_header') }}</th>
