<div {{ $attributes->merge(['class' => 'flex items-center justify-end gap-[21px] z-[60]']) }}>
    {{-- Язык --}}
    <livewire:app.language-switcher />
    ...
    @auth
        ...
        <button type="button"
            ...
            <span x-cloak x-show="$store.notifications.unread > 0" x-text="$store.notifications.unread"
        ...
        <button type="button" x-on:click="isSettingsModal=true; $store.menu.open = false; console.log(isSettingsModal)"
            class="flex items-center justify-center rounded-[6px]
                       bg-transparent hover:bg-[#232347] transition-colors">
            <img src="{{ vite()->icon('/actions/settings.svg') }}" alt="{{ __('settings') }}" class="w-[24px] h-[24px]" />
        </button>
        <div class="flex items-center gap-[8px]">
            ...
            <div class="flex items-center gap-[21px]">
                ...
                <form action="{{ route('logout') }}" method="POST" class="inline">
                    @csrf
                    <button type="submit"
                        class="flex items-center justify-center rounded-[6px] bg-transparent hover:bg-[#232347] transition-colors">
                        <img src="{{ vite()->icon('/actions/logout.svg') }}" alt="Выход"
                            class="w-[24px] h-[24px] text-[#B4FF59]" />
                    </button>
                </form>
            </div>
        </div>
