...
<x-widget.modal condition-name="$wire.isModalToPackage" maxWidth="md"
    class="p-6 w-auto md:w-[440px] space-y-8 max-h-[90vh] overflow-y-auto">

    <div class="flex justify-between items-center">
        <h2 class="font-dela text-[20px] text-white">
            {{ __('components_account_itc_partners_package_deposit_to_package') }} 111</h2>
    ...
    <div class="space-y-4">
        ...
        <div class="flex items-center overflow-x-hidden md:max-h-[460px] max-h-[380px] flex-col gap-3 overflow-y-auto p-1 px-4 md:px-8 md:mx-0 mx-[-30px]"
            ...
            @foreach ($this->packagesForTopup as $pkg)
                <label class="relative block cursor-pointer mb-[20px]"
                    :class="sel === '{{ $pkg->uuid }}'
                        ?
