...
class PromoCodeIndexPage extends IndexPage
{
    ...
    public function fields(): array
    {
        return [
            ...
            Enum::make('Тип пакета', 'package_type')->attach(PackageTypeEnum::class)->showOnExport(),
    ...
    public function components(): array
    {
        ...
        $logsData = PromoCodeUsage::with(['user', 'promoCode'])
            ->orderByDesc('used_at')
            ->orderByDesc('id')
            ->get()
            ->map(fn (PromoCodeUsage $usage) => [
                ...
                'package_type' => $usage->package_type?->getName() ?? '-',
                'used_at' => $usage->used_at?->format('d.m.Y H:i:s') ?? '-',
            ])
            ->toArray();

        $listData = PromoCode::with(['usages', 'createdByAdmin'])
            ->orderByDesc('created_at')
            ->orderByDesc('id')
            ->get()
            ->map(fn (PromoCode $item) => [
                'code' => $item->code,
                'package_type' => $item->package_type?->getName() ?? '-',
                    ...
                    ->fields([
                        ...
                        TableBuilder::make()
                            ...
                            ->fields([
                                ...
                                Text::make('Тип пакета', 'package_type')->showOnExport(),
