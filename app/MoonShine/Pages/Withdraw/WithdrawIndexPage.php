...
class WithdrawIndexPage extends IndexPage
{
    ...
    public function fields(): array
    {
        return [
            ...
            Text::make('Адрес', formatted: static function (Withdraw $item): string {
                if ($item->payment_source_id === 2 && $item->fiatDetail) {
                    return $item->fiatDetail->sbp_phone ?? '';
                }

                return $item->wallet_address ?? '';
            })->showOnExport(),
            Text::make('Карта', formatted: static function (Withdraw $item): string {
                if ($item->payment_source_id === 2 && $item->fiatDetail) {
                    return $item->fiatDetail->bank_name ?? '';
                }

                return '';
            })->showOnExport(),
            Text::make('Счёт', formatted: static function (Withdraw $item): string {
                if ($item->payment_source_id === 2 && $item->fiatDetail) {
                    return $item->fiatDetail->recipient_name ?? '';
                }
